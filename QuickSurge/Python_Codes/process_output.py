...
def load_zeta_and_hs_max(pathres):
    ...
    try:
        data= netCDF4.Dataset(pathres+ 'swan_TPS_max.63.nc')
        tp_max = np.array(data['swan_TPS_max'])
    except:
        tp_max = []
        
    try:
        data= netCDF4.Dataset(pathres+ 'swan_DIR_max.63.nc')
        dp_max = np.array(data['swan_DIR_max'])
    except:
        dp_max = []
    ...
    return(triang,lon,lat,zeta_max,hs_max,tp_max,dp_max,zeta,hs,tp,dire)
...
def get_target_lon_lat_angle(output_locations_csv):
    output_pts = pd.read_csv(output_locations_csv, delimiter=',')
    target_lat = output_pts['lat']
    target_lon = output_pts['lon']
    target_angle = output_pts['angle']
    return (target_lon, target_lat, target_angle)
...
def get_zeta_and_hs_at_point_locations(zeta_max,hs_max,tp_max,dp_max,posix):
    zeta_output = np.zeros(np.size(posix))
    hs_output = np.zeros(np.size(posix))
    tp_output = np.zeros(np.size(posix))
    dp_output = np.zeros(np.size(posix))
    for i in range(len(posix)):
        zeta_output[i] = zeta_max[posix[i]]
        hs_output[i] = hs_max[posix[i]]
        tp_output[i] = tp_max[posix[i]]
        dp_output[i] = dp_max[posix[i]]
    return(zeta_output,hs_output,tp_output,dp_output)

def calc_max_TWL_nearshore_based_on_Merrifield(hs_ts,tp_ts,dp_ts,zeta_ts):
    ...
    index_max = np.nanargmax(TWL_guess)
    
    return(max_TWL_guess,Hs[index_max],Per[index_max],Dir[index_max])


def calc_max_TWL_nearshore_based_on_Merrifield_angle(hs_ts, tp_ts, dp_ts, zeta_ts, theta_N):
    ...
    angled = Dir-theta_N
    if theta_N<=90:
        angled[angled>270] = angled[angled>270]-360;

    if theta_N>=270:
        angled[angled<-270] = angled[angled<-270]+360;
 
    angles = np.abs(np.cos(np.deg2rad(angled)))
    # ix = np.where(angles < 0)[0]
    # angles[ix] = 0
    Hb = np.zeros(np.shape(hs_ts))
    RU_2_perc = np.zeros(np.shape(hs_ts))
    for i in range(len(Hb)):
        Hb[i] = (Hs[i]**2. * Per[i] * (4*np.pi)**(-1)
                 * angles[i] * np.sqrt(1.0*9.81))**(2/5)
        RU_2_perc[i] = 0.33 * Hb[i] + (-0.1)
...
def store_output_max_at_point_locations_angle(pathres,output_locations_csv,outfilename):
...
# output_locations_csv = input_folder + 'TO.csv'
    ...
    triang,lon,lat,zeta_max,hs_max,tp_max,dp_max,zeta,hs,tp,dire = load_zeta_and_hs_max(pathres)
    target_lon,target_lat, target_angle = get_target_lon_lat_angle(output_locations_csv)
    
    posix = [get_ix_of_min_dist(lon, lat, target_lon[i], target_lat[i])
             for i in range(len(target_lon))]

    zeta_output, hs_output, tp_output, dp_output = get_zeta_and_hs_at_point_locations(
        zeta_max, hs_max, tp_max, dp_max, posix)

    max_TWL_guess = np.zeros(np.shape(posix))
    max_hs = np.zeros(np.shape(posix))
    max_tp = np.zeros(np.shape(posix))
    max_dp = np.zeros(np.shape(posix))
    ...
    for i in range(len(posix)):
        ...
        theta_N = target_angle[i]
        max_TWL_guess[i], max_hs[i], max_tp[i], max_dp[i] = calc_max_TWL_nearshore_based_on_Merrifield_angle(hs_ts, tp_ts, dp_ts, zeta_ts, theta_N)
        
    
    # make_results_folder(pathres)
    df = pd.DataFrame({"lon": target_lon, "lat": target_lat, "zeta_max[m]": zeta_output, "hs_max[m]": max_hs, "Tp_max[s]":max_tp, "Dp_max[s]":max_dp, "max_TWL_nearshore[m]": max_TWL_guess})
...
# output_locations_csv = input_folder + 'TO.csv'
    ...
    posix = [get_ix_of_min_dist(lon,lat,target_lon[i],target_lat[i]) for i in range(len(target_lon))]

    #posix = Parallel(n_jobs=12)(delayed(get_ix_of_min_dist)(lon,lat,target_lon[i],target_lat[i]) for i in range(len(target_lon)))
    zeta_output,hs_output,tp_output,dp_output = get_zeta_and_hs_at_point_locations(zeta_max,hs_max,tp_max,dp_max,posix)
    max_TWL_guess = np.zeros(np.shape(posix))
    max_hs = np.zeros(np.shape(posix))
    max_tp = np.zeros(np.shape(posix))
    max_dp = np.zeros(np.shape(posix))
    for i in range(len(posix)):
        ...
        dp_ts = dire[:,posix[i]]
        zeta_ts = zeta[:,posix[i]]
        max_TWL_guess[i], max_hs[i],max_tp[i],max_dp[i] = calc_max_TWL_nearshore_based_on_Merrifield(hs_ts,tp_ts,dp_ts,zeta_ts)
...
def plot_timser_at_point_locations(pathres,input_folder,target_locations,start_time,save):
    ...
    triang,lon,lat,zeta_max,hs_max,zeta,hs,tp,dire = load_zeta_and_hs_max(pathres)
...
def ADCIRC_SWAN_2_tiff(pathres):
    ...
    triangu = tri.Triangulation(lon,lat,triang-1)
    
    # Create regular grid
    loninc= np.round((lon.max()-lon.min())/0.005)
    latinc= np.round((lat.max()-lat.min())/0.005)
    
    
    lgrid_x, lgrid_y = np.mgrid[lon.min():lon.max():(loninc*1j),lat.min():lat.max():(latinc*1j)]
    ...
    # Interpolation for Hs_max
    intsurf = tri.LinearTriInterpolator(triangu,hs_max)
    Hsmax = intsurf(lgrid_x, lgrid_y)
    
    grid2tiff(lgrid_x,lgrid_y,Hsmax,pathres+ '/results/hs_max.tiff')
    
    # Interpolation for Zmax (storm tide)
    intsurf = tri.LinearTriInterpolator(triangu,zeta_max)
    Zmax = intsurf(lgrid_x, lgrid_y)
    
    grid2tiff(lgrid_x,lgrid_y,Zmax,pathres+ '/results/zeta_max.tiff')
    
def grid2tiff(lgrid_x,lgrid_y,var,fullfilepath):
