...
class ConfigPlugIn:
    ...
    def __init__(self, json_config):
        ...
        # Creating some missing fields in the configuration files
        if 'trackFile' in self.config.keys():
            self.config['tracks_file'] = "/TCHA/"+self.config['trackFile']
        
        if 'trackSource' in self.config.keys():
            self.config['tracks_source'] = self.config['trackSource']
            
        
        if 'domainCode' in self.config.keys():
            self.config['country_code'] = self.config['domainCode']
        
        
        if 'cyclone_name' not in self.config.keys():
            dfmt = "%Y%m%dT%H%M%SZ"
            tstamp = datetime.utcnow().strftime(dfmt)
            
            name = '{tstamp}_{cnt_code}_{tc_name}_{tc_year}'
            self.config['cyclone_name'] = name.format(tstamp=tstamp,
                                                      cnt_code=self.config['domainCode'],
                                                      tc_name=self.config['stormName'],
                                                      tc_year=self.config['stormYear'])
    ...
    def get_output_dir(self, items=[]):
        ...
        #formatted_time = tnow.strftime("%Y-%m-%dT%H:%M:%SZ")
        #dirname = self.config['cyclone_name'] +"_"+ formatted_time
...
def setup(fname):
    ...
    # `tcevent` module
    ConfigRun = ConfigPlugIn(json_config=fname)
...
if __name__ == "__main__":
    root = os.path.split(os.path.dirname(__file__))[0]
    run = get_input_file(root)[0]
    fname = get_input_file(root)[1]
    if run:
        setup(fname)
    else:
        print(get_input_file(root)[2])
