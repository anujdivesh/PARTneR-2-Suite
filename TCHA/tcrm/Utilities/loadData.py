...
def format_RSMC_track_files(fpath, fout, eez_path, dfmt):
    ...
    to run is generated in the temporary folder.
    
    Note that the rmax values are calculated and added to the new csv.

    Parameters
    ----------
    fpath : Str
        Path-like str pointing to the RMSC tracks file.
        
    fout : Str
        Path-like str pointing to the temporary folder.
